...
from trade_lib import (SHARES_PER_LOT, RiskConfig, r_multiple,  # noqa: E402
                       tick_size)
    ...
    for ev in events:
        ...
        if t == "equity":
            ...
            if sym in positions:
                ...
                raise SystemExit(f"[!!] line {ev['_line']}: {sym} already open. "
            ...
            if sym not in positions:
                raise SystemExit(f"[!!] line {ev['_line']}: stop for {sym}, which is not open")
            ...
            sym = ev["symbol"]
            if sym not in positions:
                raise SystemExit(f"[!!] line {ev['_line']}: {t} for {sym}, which is not open")
            pos = positions[sym]
            sold = abs(int(ev["lots"]))
            if sold > pos["lots"]:
                raise SystemExit(f"[!!] line {ev['_line']}: {t} {sold} lots of {sym} "
            ...
            if pos["lots"] == 0 or t == "close":
                if pos["lots"] != 0 and t == "close":
                    raise SystemExit(f"[!!] line {ev['_line']}: close of {sym} leaves "
                                     f"{pos['lots']} lots open — use 'scale' for partials")
                rec = dict(pos)
                rec.update(exit_px=px, exit_reason=ev.get("reason", ""),
                           closed=ev["ts"][:10], realised_r_total=pos["realised_idr"] / pos["r_idr"]
            ...
            raise SystemExit(f"[!!] line {ev['_line']}: unknown event type {t!r}")
...
def verify() -> int:
    ...
    for ev in events:
        ...
        if ev.get("type") not in EVENT_TYPES:
            problems.append(f"line {ev['_line']}: bad type {ev.get('type')!r}")
        if ts < last_ts:
            problems.append(f"line {ev['_line']}: timestamp {ts} precedes previous {last_ts}")
        last_ts = max(last_ts, ts)
        if ev.get("type") == "open":
            need = ("symbol", "lots", "entry_px", "stop_px", "r_idr")
            for k in need:
                if ev.get(k) in (None, ""):
                    problems.append(f"line {ev['_line']}: open missing {k}")
            if all(ev.get(k) is not None for k in need):
                ...
                if abs(want - float(ev["r_idr"])) > 1.0:
                    problems.append(
                        f"line {ev['_line']}: r_idr {float(ev['r_idr']):,.0f} does not match "
                        f"(entry-stop)*lots*100 = {want:,.0f} — the risk was silently rewritten")
                if float(ev["stop_px"]) >= float(ev["entry_px"]):
                    problems.append(f"line {ev['_line']}: stop at or above entry")
                ...
                sp = float(ev["stop_px"])
                if sp % tick_size(sp):
                    problems.append(f"line {ev['_line']}: stop {sp:g} is not a valid tick "
                                    f"(band tick is {tick_size(sp)}) — the exchange will "
                                    f"reject that order")
                if int(ev["lots"]) <= 0:
                    problems.append(f"line {ev['_line']}: non-positive lots")
...
def cmd_open(a) -> int:
    ...
    fee = a.fee if a.fee is not None else a.px * a.lots * SHARES_PER_LOT * cfg.fee_buy
    r_idr = (a.px - stop) * a.lots * SHARES_PER_LOT
    ev = {"ts": now_wib(), "type": "open", "symbol": a.symbol, "date": a.date or today_wib(),
          "lots": a.lots, "entry_px": a.px, "fee_idr": round(fee, 2),
          "stop_px": stop, "stop_basis": basis or "manual", "atr14": atr14,
          "r_idr": r_idr, "intended_risk_idr": cfg.risk_budget_idr,
          "sector": sector, "beta": beta_, "entry_rule": rule or "",
          "plan_id": f"{a.date or today_wib()}:{a.symbol}"}
    append_event(ev)
...
def cmd_stop(a) -> int:
    ...
    append_event({"ts": now_wib(), "type": "stop", "symbol": a.symbol, "stop_px": a.px,
                  "prev_stop_px": pos["stop_px"], "reason": a.reason or ""})
...
def _exit_event(a, kind: str) -> int:
    ...
    fee = a.fee if a.fee is not None else a.px * lots * SHARES_PER_LOT * cfg.fee_sell
    pnl = (a.px - pos["entry_px"]) * lots * SHARES_PER_LOT - fee
    leg_r = pos["r_idr"] * lots / pos["lots_initial"]
    append_event({"ts": now_wib(), "type": kind, "symbol": a.symbol, "lots": -lots,
                  "px": a.px, "fee_idr": round(fee, 2), "reason": a.reason or "",
